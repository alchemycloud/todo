...
@Component({
  selector: 'user-role-drop-down',
  templateUrl: './userRoleDropDown.dropdown.html',
  styleUrls: ['./userRoleDropDown.dropdown.scss'],
  providers: [
    {
      provide: MatFormFieldControl,
      useExisting: UserRoleDropDown
    }
  ]
})
export class UserRoleDropDown implements OnInit, AfterViewInit, ControlValueAccessor, DoCheck, MatFormFieldControl<UserRole>, OnDestroy {
  ...
  public filtered: Array<UserRole>;
  ...
  ngOnInit(): void {
    ...
    this.model = Object.keys(UserRole).map((key) => {
      return UserRole[key];
    });
  ...
  static nextId = 0;
  @HostBinding() id = `user-role-drop-down-${UserRoleDropDown.nextId++}`;
