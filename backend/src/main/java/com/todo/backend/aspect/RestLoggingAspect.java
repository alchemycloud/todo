...
public class RestLoggingAspect extends AbstractProfiledAspect {
    ...
    @Around(
            "(execution(public * com.todo.backend.rest.*ApiResource.*(..))"
                    + ")"
                    + " && !execution(public * com.todo.backend.rest.AuthenticationApiResource.*(..))")
    public Object doBasicProfiling(ProceedingJoinPoint pjp) throws Throwable {
        return logApiCall(pjp, WARN);
    }

    @Around("execution(public * com.todo.backend.rest.AuthenticationApiResource.*(..))")
